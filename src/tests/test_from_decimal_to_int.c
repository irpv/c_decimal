...
START_TEST(dec_to_i_1) {
  ...
  int origin_error = 1;
  int my_error = s21_from_decimal_to_int(src, NULL);
  ck_assert_int_eq(origin_error, my_error);
}
END_TEST

Suite *suite_from_decimal_to_int() {
  Suite *s = suite_create(YELLOW_BG BLACK "s21_from_decimal_to_int" RESET);
  TCase *tc = tcase_create("decimal_from_decimal_to_int_tc");
